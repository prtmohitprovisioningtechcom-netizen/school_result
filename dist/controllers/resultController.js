...
exports.downloadMarksheets = exports.downloadAdmitCards = exports.generateResults = void 0;
...
const termKey = (name) => {
    ...
    if (n.includes('terminal') || n.includes('unit'))
        return 'terminal';
    if (n.includes('half'))
        return 'half';
...
const cell = (v, absent) => (absent ? 'ABS' : fmt(v));
// =======================
// GENERATE RESULTS
// =======================
const generateResults = async (req, res) => {
    try {
        ...
        const exams = await (0, dbHelper_1.query)(`SELECT id, max_marks, passing_marks FROM exams
       WHERE session_id = ? AND exam_type_id = ? AND class_id = ?`, [session_id, exam_type_id, class_id]);
        if (exams.length === 0) {
            return res.status(400).json({ message: 'No exams found for the selected criteria' });
        }
        const examIds = exams.map((e) => e.id);
        const placeholders = examIds.map(() => '?').join(',');
        const marksData = await (0, dbHelper_1.query)(`SELECT student_id,
              SUM(
                CASE
       ...
       GROUP BY student_id`, examIds);
        const totalMaxMarks = exams.reduce((sum, e) => sum + parseFloat(e.max_marks || 0), 0);
        const grades = await (0, dbHelper_1.query)('SELECT * FROM grades ORDER BY min_percentage DESC');
        await (0, dbHelper_1.withTransaction)(async (connection) => {
            for (const row of marksData) {
                ...
                await connection.execute(`INSERT INTO results (student_id, session_id, exam_type_id, total_max_marks, total_marks_obtained, percentage, grade, pass_fail)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?)
           ...
           grade = VALUES(grade),
           pass_fail = VALUES(pass_fail)`, [studentId, session_id, exam_type_id, totalMaxMarks, totalObtained, percentage.toFixed(2), gradeName, passFail]);
            }
            await connection.execute(`UPDATE results r
         JOIN (
         ...
         SET r.\`rank\` = ranked.rnk`, [session_id, exam_type_id, class_id]);
        });
...
// =======================
const downloadAdmitCards = async (req, res) => {
    var _a, _b, _c;
    try {
        const settingsArr = await (0, dbHelper_1.query)('SELECT * FROM school_settings LIMIT 1');
        const settings = settingsArr[0] || { school_name: 'School ERP' };
        const { session_id, exam_type_id, class_id } = req.body;
        if (!session_id || !exam_type_id || !class_id) {
            return res.status(400).json({ message: 'session_id, exam_type_id and class_id are required' });
        }
        const students = await (0, dbHelper_1.query)(`SELECT s.id, s.first_name, s.last_name, s.roll_number, s.admission_number, s.photo_url, s.dob, s.gender,
       ...
       WHERE s.class_id = ?
       ORDER BY CAST(s.roll_number AS UNSIGNED), s.first_name`, [session_id, class_id]);
        ...
        const schedule = await (0, dbHelper_1.query)(`SELECT e.exam_date, e.max_marks, sub.name AS subject_name, et.name AS exam_type_name
       FROM exams e
       JOIN subjects sub ON e.subject_id = sub.id
       JOIN exam_types et ON e.exam_type_id = et.id
       WHERE e.session_id = ? AND e.exam_type_id = ? AND e.class_id = ?
       ORDER BY e.exam_date ASC, sub.name ASC`, [session_id, exam_type_id, class_id]);
        const schoolRows = await (0, dbHelper_1.query)('SELECT * FROM school_settings LIMIT 1');
        const school = schoolRows[0] || {};
        const examTypeName = ((_a = schedule[0]) === null || _a === void 0 ? void 0 : _a.exam_type_name) ||
            ((_b = (await (0, dbHelper_1.query)('SELECT name FROM exam_types WHERE id = ?', [exam_type_id]))[0]) === null || _b === void 0 ? void 0 : _b.name) ||
            'Examination';
        const sessionName = ((_c = students[0]) === null || _c === void 0 ? void 0 : _c.session_name) || '';
        ...
        <tr>
          ...
          <td>${(_a = row.max_marks) !== null && _a !== void 0 ? _a : '-'}</td>
          <td></td>
        </tr>`;
        })
            .join('');
    ...
    `;
      ...
      `;
        ...
        const pdfBuffer = await (0, pdfGenerator_1.generatePDFFromHTML)(htmlContent, 'A4', false);
...
exports.downloadAdmitCards = downloadAdmitCards;
// =======================
...
const downloadMarksheets = async (req, res) => {
    ...
    try {
        const settingsArr = await (0, dbHelper_1.query)('SELECT * FROM school_settings LIMIT 1');
        const settings = settingsArr[0] || { school_name: 'School ERP' };
        const userRole = (_a = req.user) === null || _a === void 0 ? void 0 : _a.role_id;
       ...
       LEFT JOIN academic_sessions sess ON sess.id = ?
       WHERE s.class_id = ?`;
        const queryParams = [session_id, class_id];
        if (student_id) {
            queryStr += ` AND s.id = ?`;
            queryParams.push(student_id);
        }
        queryStr += ` ORDER BY CAST(s.roll_number AS UNSIGNED), s.first_name`;
        const students = await (0, dbHelper_1.query)(queryStr, queryParams);
        if (students.length === 0) {
            return res.status(404).json({ message: 'No students found for this class/session' });
        }
        const markRows = await (0, dbHelper_1.query)(`SELECT m.student_id, m.marks_ist, m.marks_iind, m.marks_obtained, m.grace_marks, m.is_absent,
       ...
       ORDER BY sub.name ASC`, [session_id, class_id]);
        if (markRows.length === 0) {
            return res.status(404).json({
                message: 'No marks found. Enter marks for Terminal / Half-Yearly / Annual exams first.'
            });
        }
        const results = await (0, dbHelper_1.query)(`SELECT r.*, et.name AS exam_type_name
       FROM results r
       JOIN exam_types et ON r.exam_type_id = et.id
       JOIN students s ON r.student_id = s.id
       WHERE r.session_id = ? AND s.class_id = ?`, [session_id, class_id]);
        const schoolRows = await (0, dbHelper_1.query)('SELECT * FROM school_settings LIMIT 1');
        const school = schoolRows[0] || {};
        const sessionName = ((_b = students[0]) === null || _b === void 0 ? void 0 : _b.session_name) || '';
    ...
    `;
        for (const st of students) {
            const studentMarks = markRows.filter((m) => m.student_id === st.id);
            const studentResults = results.filter((r) => r.student_id === st.id);
            const byTermSubject = {
                terminal: {},
                half: {},
                annual: {},
            ...
            <tr>
              ...
              <td>${fmt(obtained)}</td><td>${fmt(maximum)}</td>
                ...
                .join('');
            const pct = (obt, max) => (max > 0 ? ((obt / max) * 100).toFixed(2) : '-');
            const resultOf = (obt, max) => {
                if (max <= 0)
                    return '-';
                return (obt / max) * 100 >= 33 ? 'Pass' : 'Fail';
            };
            const findResult = (needle) => studentResults.find((r) => termKey(r.exam_type_name) === needle);
        ...
        <div class="sheet">
          ...
          <div class="bottom">
            <div class="summary">
              <div class="box">
                <h4>TERMINAL EXAM</h4>
                <div>Percentage: <strong>${(_c = tRes === null || tRes === void 0 ? void 0 : tRes.percentage) !== null && _c !== void 0 ? _c : pct(tTot, tMax)}%</strong></div>
                <div>Position: <strong>${(_d = tRes === null || tRes === void 0 ? void 0 : tRes.rank) !== null && _d !== void 0 ? _d : '-'}</strong></div>
                <div>Result: <strong>${(_e = tRes === null || tRes === void 0 ? void 0 : tRes.pass_fail) !== null && _e !== void 0 ? _e : resultOf(tTot, tMax)}</strong></div>
              </div>
              <div class="box">
                <h4>HALF YEARLY EXAM</h4>
                <div>Percentage: <strong>${(_f = hRes === null || hRes === void 0 ? void 0 : hRes.percentage) !== null && _f !== void 0 ? _f : pct(hTot, hMax)}%</strong></div>
                <div>Position: <strong>${(_g = hRes === null || hRes === void 0 ? void 0 : hRes.rank) !== null && _g !== void 0 ? _g : '-'}</strong></div>
                <div>Result: <strong>${(_h = hRes === null || hRes === void 0 ? void 0 : hRes.pass_fail) !== null && _h !== void 0 ? _h : resultOf(hTot, hMax)}</strong></div>
              </div>
              <div class="box">
                <h4>ANNUAL EXAM</h4>
                <div>Percentage: <strong>${(_j = aRes === null || aRes === void 0 ? void 0 : aRes.percentage) !== null && _j !== void 0 ? _j : pct(aTot, aMax)}%</strong></div>
                <div>Position: <strong>${(_k = aRes === null || aRes === void 0 ? void 0 : aRes.rank) !== null && _k !== void 0 ? _k : '-'}</strong></div>
                <div>Result: <strong>${(_l = aRes === null || aRes === void 0 ? void 0 : aRes.pass_fail) !== null && _l !== void 0 ? _l : resultOf(aTot, aMax)}</strong></div>
              </div>
            </div>
      ...
      `;
        ...
        htmlContent += '</body></html>';
        const pdfBuffer = await (0, pdfGenerator_1.generatePDFFromHTML)(htmlContent, 'A4', true);
        res.setHeader('Content-Type', 'application/pdf');
...
exports.downloadMarksheets = downloadMarksheets;
